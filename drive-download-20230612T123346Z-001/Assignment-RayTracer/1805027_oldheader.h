...
public:
    ...
    //             Vector3D lightPosition = pointLights[i]->pos;
    ...
    //              Vector3D normal = getNormal(intersectionPoint); //need to change
    //              normalize(normal);
    ...
    //             double t2 = sqrt(dot((intersectionPoint - lightPosition),(intersectionPoint - lightPosition)));
    //             if(t2 < 1e-5) continue;

    //             bool obscured = isObscuredPointLight(pointLights[i], intersectionPoint);

    //             if(!obscured){
                    
    //                 // lambert value
    //                 lightray.dir.x = -lightray.dir.x;
    //                 lightray.dir.y = -lightray.dir.y;
    //                 lightray.dir.z = -lightray.dir.z;

    //                 double val = max(0.0, dot(lightRay.dir, normal));
                    
    //                 // find reflected ray
    //                 Ray reflection = Ray(intersectionPoint, lightRay.dir - normal*2*(lightRay.dir*normal));
    //                 ray.dir.x = -ray.dir.x;
    //                 ray.dir.y = -ray.dir.y;
    //                 ray.dir.z = -ray.dir.z;
    //                 double phong = max(0.0,dot(ray.dir,reflection.dir));
    ...
    //             Vector3D lightPosition = spotLights[i]->pos;
    //             Vector3D lightDirection = intersectionPoint - lightPosition;
    //             normalize(lightDirection);
