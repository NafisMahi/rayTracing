...
void drawHalfOneSixthSphere()
{
    glBegin(GL_POLYGON);
       std::vector<float> vertices = verticesCubeSphere(8);
       for(int i = 0;i < vertices.size()/2.0; i+=3)
       {
         glVertex3f(vertices[i],vertices[i+1],vertices[i+2]);
       }
    glEnd();
...
void drawHalfScaledFaceOfSphere()
{
    glPushMatrix();
        glTranslated((16.0 - stepno)/16.0,0.0,0.0);
        glScaled(sphereScaleFactor/16.0,sphereScaleFactor/16.0,sphereScaleFactor/16.0);
        glScaled(1/sqrt(3.0),1/sqrt(3.0),1/sqrt(3.0));
        drawHalfOneSixthSphere();
    glPopMatrix();   
}
...
void glDrawHalfSphere()
{
    glColor3f(0.0f,1.0f,0.0f);
    drawHalfScaledFaceOfSphere();
...
void draw_halfCylinder(GLfloat radius,
                   GLfloat height,
                   GLubyte R,
                   GLubyte G,
                   GLubyte B)
...
void drawScaledHalfCylinder()
{
    glPushMatrix();
        glTranslated((16 - stepno)/(2*16),(16 - stepno)/(2*16),0.0);
        glRotatef(45.0,0,0,1);
        glRotatef(35.3,0,1,0);
        draw_halfCylinder(cylinderRadius,cylinderHeight,255,140,200);
    glPopMatrix();
}
