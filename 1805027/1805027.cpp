...
Color calculateColorCoefficients(Vector3D intersectionPoint, Color intersectionPointColor, Vector3D normal, Object *object)
{
    Color color;
    ...
    if (object == NULL)
    {
        color = intersectionPointColor * checkerBoard_color.r;
    }
    ...
    {
        if (!isObscuredPointLight(pointLights[i], intersectionPoint + (intersectionPoint - pointLights[i]->pos))*0.001)
        {
            ...
            Vector3D reflected = reflectedRay(toSource, normal);
            if (object != NULL)
            {
    ...
    {
        if (!isObscuredSpotLight(spotLights[i], intersectionPoint + (intersectionPoint - pointLights[i]->pos))*0.001)
        {
            // cout << " in spot light " << endl;
    ...
    for (int i = 0; i < screenWidth; ++i)
    {
        for (int j = 0; j < screenHeight; ++j)
        {
            ...
            if (nearestObject != NULL)
            {
                ...
                color = calculateColorCoefficients(ray.start + ray.dir * min_t, color, normal, nearestObject);
            ...
            else if (isfloor)
            {
                ...
                calculateColorCoefficients(ray.start + ray.dir * min_t, color, normal, NULL);
                image.set_pixel(j, i, 255 * color.r, 255 * color.g, 255 * color.b);
            }
            else
            {
